...
@Getter
@Slf4j
public class QtConfiguration extends AbstractOperatorConfiguration {
  static final String CONFIG_PREFIX = "extw.operator.qt";
  ...
  public Configuration configure(Config config) {
    String passKey =
        value(CONFIG_PREFIX, "pass-key", String.class, UUID.randomUUID().toString(), config);
    ...
    hashKey = value(CONFIG_PREFIX, "hash-key", String.class, UUID.randomUUID().toString(), config);
    providerId =
        value(CONFIG_PREFIX, "provider-id", String.class, UUID.randomUUID().toString(), config);
    ...
    for (Long companyId : companyIds) {
      ...
      String companyPassKey =
          value(CONFIG_PREFIX, "co.%s.pass-key", String.class, "", config, companyId.toString());

      if (CommonUtils.isEmptyOrNull(companyPassKey)) {
        companyPassKey = passKey;
      }
